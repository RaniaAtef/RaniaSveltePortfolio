...
"Utilizes React, Context for state management, and JavaScript"],
	...
	function handleKeydown(event: KeyboardEvent, tabId: string): void {
		const tabs = projectCategories.map(cat => cat.id);
...
<section id="work" class="py-12 sm:py-20 px-4 bg-[#161B22] scroll-mt-20">
	<div class="max-w-6xl mx-auto">
		...
		<!-- Tab Navigation -->
		<div class="flex flex-wrap justify-center gap-2 mb-8 sm:mb-12" role="tablist" aria-label="Project categories">
			{#each projectCategories as category}
				<button
					class="flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-[#161B22] {activeTab === category.id 
		...
		<!-- Projects Grid -->
		{#if activeCategory}
			<div id="projects-panel" class="space-y-8 sm:space-y-12" role="tabpanel" aria-labelledby="projects-tab">
				{#each activeCategory.projects as project}
					<article class="bg-[#21262D] rounded-lg overflow-hidden transform hover:scale-[1.01] transition-all duration-300 hover:shadow-2xl focus-within:scale-[1.01] focus-within:shadow-2xl">
						<div class="grid grid-cols-1 lg:grid-cols-2">
							<div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
								...
								<header class="space-y-3">
									...
									<div class="flex flex-wrap gap-2" role="list" aria-label="Technologies used">
										{#each project.techStack as tech}
											<span class="px-2 py-1 text-xs font-medium bg-[#1A1F2B] text-blue-400 rounded-full border border-[#2D333B]" role="listitem">
												{tech}
											</span>
										{/each}
									</div>
								...
								<!-- Project Links -->
								<nav class="flex gap-3 pt-2" aria-label="Project links for {project.title}">
									<a 
										href={project.liveUrl} 
										class="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-[#21262D]"
										target="_blank"
									...
									<a 
										...
										class="inline-flex items-center gap-2 px-4 py-2 bg-[#1A1F2B] hover:bg-[#2D333B] text-gray-300 border border-[#2D333B] rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-[#21262D]"
										target="_blank"
										rel="noopener noreferrer"
							...
							<div class="relative h-full min-h-[300px] lg:min-h-full">
								<img 
									src={project.image} 
									alt="Screenshot of {project.title} project showing the user interface and design" 
									class="absolute inset-0 w-full h-full object-cover" 
									loading="lazy"
								/>
								<div class="absolute inset-0 bg-gradient-to-b lg:bg-gradient-to-r from-[#21262D] via-transparent to-transparent lg:via-[#21262D]/20 lg:to-[#21262D]/40" aria-hidden="true"></div>
							</div>
						</div>
					</article>
				{/each}
